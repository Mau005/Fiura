...
from configuration.constants import LIMIT_VIEW_Y, LIMIT_VIEW_X, LIMIT_VIEW_PLAYER_X, LIMIT_VIEW_PLAYER_Y, TypeObject
...
class Render(Widget):
    def __init__(self, size_internal, **kwargs):
        ...
        self.map.set_position_map(Coordinates(0, 1, 0), 2)
        self.map.set_position_map(Coordinates(2, 0, 0), 2)
        self.map.set_position_map(Coordinates(3, 0, 0), 2)
        self.map.set_position_map(Coordinates(4, 0, 0), 4)
        self.map.set_position_map(Coordinates(5, 0, 0), 4)
        self.map.set_position_map(Coordinates(6, 0, 0), 4)
        self.map.set_position_map(Coordinates(7, 0, 0), 4)
        self.map.set_position_map(Coordinates(4, 4, 0), 4)
        self.map.set_position_map(Coordinates(5, 5, 0), 1)
        self.map.set_position_map(Coordinates(6, 6, 0), 4)
        self.map.set_position_map(Coordinates(7, 7, 0), 4)
        ...
        for x in range(self.map.get_len_map_x()):
            for y in range(self.map.get_len_map_y(x)):
                ...
                if type_object == TypeObject.NULL:
                    ...
                    floor = Floor(self.manager_object, id_items,
